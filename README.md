...
Flow: wallet on disk → faucet → smoldot (mainnet preset) → register → persist `agentId`. If `lastRegisteredAgentId` is already stored, the next run skips faucet and registration.
